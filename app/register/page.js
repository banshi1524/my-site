...
export default function Register() {
  ...
  const verifyCode = async (e) => {
    ...
    if (err) setError('验证码错误或已过期，请重新获取');
      ...
      <div className="bg-white p-8 rounded-xl shadow-md w-full max-w-md">
        ...
        {!codeSent ? (
          ...
          <form onSubmit={verifyCode} className="space-y-4">
            <p className="text-sm text-gray-600">验证码已发送到 <strong>{email}</strong>，请查收</p>
            <div>
            ...
            <button type="button" onClick={() => { setCodeSent(false); setCode(''); }}
              className="w-full text-sm text-blue-600 hover:underline">← 换邮箱重新发送</button>
          </form>
        )}
